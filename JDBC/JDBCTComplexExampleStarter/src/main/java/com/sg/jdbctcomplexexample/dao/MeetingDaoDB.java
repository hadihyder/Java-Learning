...
public class MeetingDaoDB implements MeetingDao{
    ...
    public List<Meeting> getMeetingsForEmployee(Employee employee) {
        final String SELECT_MEETINGS_FOR_EMPLOYEE = "SELECT * FROM meeting m JOIN meeting_employee me ON "
                                        + "m.id = me.meetingId WHERE me.employeeId = ?";
    ...
    private List<Employee> getEmployeesForMeeting(Meeting meeting) {
        final String SELECT_EMPLOYEES_FOR_MEETING = "SELECT e.* FROM employee e JOIN meeting_employee me ON "
                + "e.id = me.employeeId WHERE me.meetingId = ?";
