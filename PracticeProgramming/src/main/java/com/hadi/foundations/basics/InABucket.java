package com.hadi.foundations.basics;

public class InABucket {
    public static void main(String[] args) {

        String walrus;
        double piesEaten;
        float weightOfTeacupPig;
        int grainsOfSand;

        walrus = "Sir Leroy Jenkins III";
        piesEaten = 42.1;
        weightOfTeacupPig = 40.0f;
        grainsOfSand = 300;

        System.out.println("Meet my pet walrus, " + walrus);
        System.out.print("He was a bit hungry today, and ate this many pies: ");
        System.out.println(piesEaten);
        System.out.print("He has a friend Zed weighing " +weightOfTeacupPig+ " ounces,");
        System.out.println(" and he hangs out in " +grainsOfSand + " grains of sand.");

    }
}
